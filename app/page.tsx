...
type Issue={id:string;staff_id:string;issue_type:string;area:string;description:string;immediate_action:string|null;priority:string;status:string;responsible_person:string|null;due_date:string|null;completion_date:string|null;manager_comments:string|null;corrective_action:string|null;created_at:string;photo_url:string|null;issue_staff?:{name:string}|null};
...
export default function Page(){
 ...
 const [form,setForm]=useState({staff_id:'',issue_type:'Hazard / Safety',area:'Workshop',description:'',immediate_action:'',priority:'Medium'}); const [photo,setPhoto]=useState<File|null>(null);
 ...
 if(!msg)setMsg('Issue submitted successfully.'); setForm({staff_id:'',issue_type:'Hazard / Safety',area:'Workshop',description:'',immediate_action:'',priority:'Medium'});setPhoto(null);await load(); };
 ...
 return <main className="wrap">
  ...
  {tab==='report'&&<>
   <div className="card"><h2>Report an Issue</h2><p className="muted">Use this form to report hazards, quality concerns, equipment issues, workplace problems or other matters requiring follow-up.</p>
    <div className="grid">
     <div className="field"><label>Staff member</label><select value={form.staff_id} onChange={e=>setForm({...form,staff_id:e.target.value})}><option value="">Select staff member</option>{staff.filter(s=>s.active).map(s=><option key={s.id} value={s.id}>{s.name}</option>)}</select></div>
     ...
     <div className="field"><label>Area / Location</label><select value={form.area} onChange={e=>setForm({...form,area:e.target.value})}><option>Workshop</option><option>Warehouse</option><option>Office</option><option>Engineering</option><option>Yard</option><option>Vehicle</option><option>Other</option></select></div>
...
function IssueCard({issue,save}:{issue:Issue;save:(id:string,p:Record<string,unknown>)=>Promise<void>}){
 ...
 <div className="grid3">
  ...
  <div className="field"><label>Status</label><select value={x.status} onChange={e=>setX({...x,status:e.target.value})}><option>New</option><option>Under Review</option><option>Action Required</option><option>Closed</option></select></div>
