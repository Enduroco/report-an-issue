...
export default function Page(){
 ...
 const submit=async()=>{setMsg(''); const r=await fetch('/api/issues',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(form)}); const j=await r.json(); if(!r.ok){setMsg(j.error||'Could not submit issue');return;} let photoFailed=''; if(photo){const fd=new FormData();fd.append('file',photo);const p=await fetch(`/api/issues/${j.issue.id}/photo`,{method:'POST',body:fd});const pj=await p.json(); if(!p.ok)photoFailed=` Issue saved, but photo failed: ${pj.error}`;}
 setMsg(photoFailed||'Issue submitted successfully.'); setForm({staff_name:'',issue_type:'Hazard / Safety',location:'Workshop',description:'',immediate_action:'',priority:'Medium'});setPhoto(null);const camera=document.getElementById('cameraPhoto') as HTMLInputElement|null;const library=document.getElementById('libraryPhoto') as HTMLInputElement|null;if(camera)camera.value='';if(library)library.value='';await load(); };
 ...
 return <main className="wrap">
  ...
  {tab==='report'&&<>
   <div className="card"><h2>Report an Issue</h2><p className="muted">Report hazards, quality concerns, equipment issues, workplace problems or other matters requiring follow-up.</p>
    ...
    <button className="btn primary full" onClick={submit}>SUBMIT ISSUE</button><div className="msg">{msg}</div>
   </div>
  </>}
  {tab==='manager'&&<>
